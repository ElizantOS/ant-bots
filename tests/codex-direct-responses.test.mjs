...
test("direct Codex Responses transport streams text without an SDK reader", async () => {
  ...
  const fetch = async (_url, init) => {
    assert.equal(init.headers.authorization, "Bearer configured-router-token");
    assert.equal(init.headers["ChatGPT-Account-Id"], "configured-account");
  ...
  const events = [];
  for await (const event of streamCodexDirectResponses({ fetch, endpoint: "https://example.invalid/responses", authorization: "Bearer configured-router-token", accountId: "configured-account", model: "gpt-test", instructions: "Grok", input: [{ role: "user", content: "hi" }] })) events.push(event);
...
test("direct Codex Responses transport overlaps parallel tool calls", async () => {
  const { streamCodexDirectResponses } = await loadModule();
  let requestCount = 0;
  let active = 0;
  let maximumActive = 0;
  const fetch = async () => {
    ...
    return requestCount === 1
      ? sse([{ type: "response.completed", response: { id: "resp-parallel", output: [
        { type: "function_call", call_id: "call-a", name: "lookup_a", arguments: "{}" },
        { type: "function_call", call_id: "call-b", name: "lookup_b", arguments: "{}" },
      ], usage: { input_tokens: 2, output_tokens: 1 } } }])
      : sse([{ type: "response.output_text.delta", delta: "done" }, { type: "response.completed", response: { id: "resp-parallel-final", output: [], usage: { input_tokens: 2, output_tokens: 1 } } }]);
  };
  for await (const _event of streamCodexDirectResponses({
    ...
    model: "gpt-test",
    instructions: "Use connected tools",
    input: [{ role: "user", content: "look up both" }],
    tools: [
      { name: "lookup_a", parameters: { type: "object" }, source: {} },
      { name: "lookup_b", parameters: { type: "object" }, source: {} },
    ],
    executeTool: async () => {
      active += 1;
