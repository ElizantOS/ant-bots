  ...
  plugins: [
    ...(isRecoveredRenderer ? [] : [react()]),
    {
      ...
      configureServer(server) {
        ...
        server.middlewares.use("/__reconstructed", sirv(pdfjsWorkerRoot, { dev: true, etag: true }));
        if (upstreamManifest != null) server.middlewares.use("/upstream", sirv(upstreamRenderer, { dev: true, etag: true }));
      }
    }
  ],
  server: {
