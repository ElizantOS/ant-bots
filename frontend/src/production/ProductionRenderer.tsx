...
import type { CoordinatorPortBridge, CursorAuthStatus, DesktopAutoReviewInstructions, DesktopBridge, InferenceProviderId, InferenceRouterSnapshot, SidebarSection, ThemePreference } from "../recovered/contracts/desktop-bridge";
...
import { SandTooltip } from "../recovered/ui/sand-floating-primitives";
...
import { BOX_STARTUP_READ_TIMEOUT_MS, parsePersistedTranscriptReplica, transcriptReplicaPersistenceKey, withBoxStartupReadTimeout } from "./degraded-startup";
...
import {
  parseDesktopIntent,
  mergeTranscriptBaselineEntries,
...
const settingsDesktopSurfaceImport = import("../recovered/features/settings/overlay/desktop-surface");
const SettingsDesktopSurface = lazy(async () => {
  const module = await settingsDesktopSurfaceImport;
...
const INFERENCE_PROVIDER_IDS: readonly InferenceProviderId[] = ["cursor", "claude-code", "codex", "opencode", "openrouter"];

function isInferenceProviderId(value: unknown): value is InferenceProviderId {
  return typeof value === "string" && INFERENCE_PROVIDER_IDS.includes(value as InferenceProviderId);
}

function parseInferenceRouterSnapshot(value: unknown): InferenceRouterSnapshot | null {
  if (typeof value !== "object" || value == null || Array.isArray(value)) return null;
  const record = value as Record<string, unknown>;
  if (!isInferenceProviderId(record.provider)) return null;
  const local = typeof record.local === "object" && record.local != null && !Array.isArray(record.local)
...
function canSendInferenceWithoutBox(snapshot: InferenceRouterSnapshot | null): boolean {
  if (snapshot == null) return false;
  if (snapshot.provider !== "cursor") return true;
  const cursor = snapshot.local?.cursor;
  return typeof cursor === "object" && cursor != null && !Array.isArray(cursor)
    && (cursor as Record<string, unknown>).authenticated === true;
}

function SettingsSidebarAction({ isCollapsed, onOpenSettings }: { readonly isCollapsed: boolean; readonly onOpenSettings: () => void }): ReactNode {
  return <SandTooltip content={UI_TEXT.settings}>
    <button
      aria-label={UI_TEXT.settings}
      className="sand-agents-sidebar__settings-button"
      data-collapsed={isCollapsed || undefined}
      onClick={onOpenSettings}
      title={UI_TEXT.settings}
      type="button"
    >
      <span aria-hidden="true" className="sand-agents-sidebar__settings-icon"><SandIcon name="gear" size="md" /></span>
      <span className="sand-agents-sidebar__settings-label">{UI_TEXT.settings}</span>
    </button>
  </SandTooltip>;
}
...
export function ProductionRenderer({ bridge, coordinatorPort }: ProductionRendererProps) {
  ...
  const [boxConnectionDegraded, setBoxConnectionDegraded] = useState(false);
  const [inferenceRouter, setInferenceRouter] = useState<InferenceRouterSnapshot | null>(null);
  ...
  const boxConnectionDegradedRef = useRef(false);
  ...
  const inferenceCanBypassBoxRef = useRef(false);
  ...
  inferenceCanBypassBoxRef.current = canSendInferenceWithoutBox(inferenceRouter);
  ...
  const markBoxConnectionDegraded = useCallback(() => {
    boxConnectionDegradedRef.current = true;
    setBoxConnectionDegraded(true);
  }, []);
  const clearBoxConnectionDegraded = useCallback(() => {
    boxConnectionDegradedRef.current = false;
    setBoxConnectionDegraded(false);
  }, []);
  ...
  const [composerSubmissionQueue] = useState(() => {
    const queue = createComposerSubmissionQueue({
      // The box powers roster/computer features, but local inference providers
      // can answer without it. Keep those sends out of the reconnect queue.
      isTransportDown: () => transportRef.current !== "connected" && !inferenceCanBypassBoxRef.current,
  ...
  useEffect(() => {
    let active = true;
    const applyRouterSnapshot = (value: unknown): void => {
      const snapshot = parseInferenceRouterSnapshot(value);
      if (!active || snapshot == null) return;
      setInferenceRouter(snapshot);
      inferenceCanBypassBoxRef.current = canSendInferenceWithoutBox(snapshot);
      if (inferenceCanBypassBoxRef.current) composerSubmissionQueueRef.current?.flush();
    };
    const onRouterChanged = (event: Event): void => {
      applyRouterSnapshot((event as CustomEvent<unknown>).detail);
    };
    window.addEventListener("sand-router-provider-changed", onRouterChanged);
    void bridge.agent.getInferenceRouter().then(applyRouterSnapshot).catch(() => {});
    return () => {
      active = false;
      window.removeEventListener("sand-router-provider-changed", onRouterChanged);
    };
  }, [bridge, composerSubmissionQueue]);
  ...
  const restorePersistedTranscripts = useCallback(async (accountSlot: string, restoredAgents: readonly RendererAgent[]) => {
    const accountGeneration = accountScopeGenerationRef.current;
    await Promise.all(restoredAgents.map(async (agent) => {
      const key = transcriptReplicaPersistenceKey(accountSlot, agent.id);
      const raw = await bridge.agent.clientPersistence.read(key).catch(() => null);
      if (accountScopeGenerationRef.current !== accountGeneration || accountRef.current?.kind !== "logged-in") return;
      const projected = projectTranscriptFeedEntries(parsePersistedTranscriptReplica(raw), agent.name, agent.id);
      if (projected.length === 0) return;
      setEntriesByAgent((current) => ({
        ...current,
        [agent.id]: mergeTranscriptEntries(current[agent.id] ?? [], projected)
      }));
    }));
  }, [bridge]);
  useEffect(() => {
    if (account?.kind !== "logged-in") return;
    const accountSlot = account.authId ?? account.email ?? "account";
    const snapshotHasRoster = accessRosterSnapshot.hasCompleteRoster && accessRosterSnapshot.agents.length > 0;
    if (snapshotHasRoster && !hasLoadedAgentsRef.current) {
      const restoredAgents = projectRendererAgents(accessRosterSnapshot.agents);
      if (restoredAgents.length > 0) {
        setAgents(restoredAgents);
        completeRosterAgentIdsRef.current = restoredAgents.map((agent) => agent.id);
        setHasLoadedAgents(true);
        selectionStore.reconcile({ agentIds: completeRosterAgentIdsRef.current, isRosterComplete: true });
        void restorePersistedTranscripts(accountSlot, restoredAgents);
      }
    }
    if (accessRosterSnapshot.failure != null) {
      markBoxConnectionDegraded();
      setRosterLoadFailed(true);
      setRosterFailure(projectRosterFailure(accessRosterSnapshot.failure));
    }
  }, [accessRosterSnapshot.agents, accessRosterSnapshot.failure, accessRosterSnapshot.hasCompleteRoster, account?.kind, markBoxConnectionDegraded, restorePersistedTranscripts, selectionStore]);
  ...
  const sidebarSections = useMemo(() => sidebarSectionRecords == null || sidebarSectionRecords.length === 0
    ? null
    : sidebarSectionRecords.map((section) => ({ ...section, isCollapsed: collapsedSectionIds.includes(section.id) })), [collapsedSectionIds, sidebarSectionRecords]);
  ...
  const resolveOnboarding = useCallback(async (status: CursorAuthStatus, hasSeenOnboarding: boolean): Promise<void> => {
    ...
    // A first-run wizard cannot complete without the computer. Keep the shell
    // usable when the box is already known to be unavailable instead of
    // reopening the wizard on every account observation.
    if (boxConnectionDegradedRef.current || transportRef.current === "down") {
      setOnboardingOpen(false);
      return;
    }
    ...
    if (!hasSeenOnboarding && client != null) {
      try {
        const result = await withBoxStartupReadTimeout(client.call("countAgents"), "countAgents");
        ...
        markBoxConnectionDegraded();
        if (accountScopeGenerationRef.current === accountScopeGeneration) setOnboardingOpen(false);
        return;
      }
    }
    ...
    if (accountScopeGenerationRef.current !== accountScopeGeneration) return;
    setOnboardingOpen(route === "onboarding" && !boxConnectionDegradedRef.current);
  }, [bridge, client, markBoxConnectionDegraded]);
  ...
  const refreshRoster = useCallback(async () => {
    ...
    try {
      const projected = projectRendererAgents(await withBoxStartupReadTimeout(client.call("listAgents"), "listAgents"));
      if (!isCurrent()) return;
      clearBoxConnectionDegraded();
  ...
  }, [clearBoxConnectionDegraded, client, markBoxConnectionDegraded, selectionStore]);
  ...
  const openAgent = useCallback(async (agentId: string) => {
    ...
    try {
      ...
      setEntriesByAgent((current) => ({ ...current, [agentId]: mergeTranscriptBaselineEntries(current[agentId] ?? [], projectedPage.entries) }));
  ...
  useEffect(() => {
    ...
    const startupFallbackTimer = setTimeout(() => {
      // The coordinator may be waiting for a local Docker VM while the account
      // status is still settling. Release the shell independently of auth so a
      // slow computer startup cannot cover the whole application indefinitely.
      if (!isCurrent() || transportRef.current !== "connecting") return;
      transportScopeGenerationRef.current += 1;
      setTransport("down");
      setRosterLoadFailed(true);
      markBoxConnectionDegraded();
    }, BOX_STARTUP_READ_TIMEOUT_MS);
    client.ready.then(() => {
    ...
    const stopTranscript = reactionRoot?.feed.observeEntriesFeed({
      onBaseline: ({ agentId: ownerId, entries: rawEntries }) => {
        ...
        const projectedEntries = mergeTranscriptBaselineEntries(
          entriesByAgentRef.current[ownerId] ?? [],
          projectTranscriptFeedEntries(rawEntries, owner?.name ?? UI_TEXT.title, ownerId),
        );
      ...
      onUpdated: ({ agentId: ownerId, after: event }) => {
        ...
        setEntriesByAgent((current) => {
          ...
          return { ...current, [ownerId]: mergeTranscriptEntries(existing, [projected]) };
        });
      },
      onCleared: (ownerId) => {
        if (!isCurrent() || accountRef.current?.kind !== "logged-in") return;
      ...
      setEntriesByAgent((current) => {
        ...
        return { ...current, [ownerId]: pendingId != null && pending != null
          ? mergeTranscriptEntries(existing.map((entry) => entry.kind === "message" && entry.id === pendingId ? projected : entry), [])
          : mergeTranscriptEntries(existing, [projected]) };
      });
      }
    }) ?? null;
    return () => {
  ...
  }, [account?.kind, client, clientLifecycleGenerationRef, connectionController, markBoxConnectionDegraded, pinnedAccountKey, reactionRoot, refreshAgentNetworkAvailability, refreshRoster, selectionStore, transcriptAccountSlot]);
  ...
  const submit = () => {
    ...
    setEntriesByAgent((current) => ({ ...current, [activeAgent.id]: mergeTranscriptEntries(current[activeAgent.id] ?? [], [{
      kind: "message", id: `pending-${clientNonce}`, role: "user", author: "You", text: prompt, timestampMs: enteredAt, attachments, delivery: "pending", clientNonce, routed: true,
      ...(submission.replyToId == null ? {} : { replyToId: submission.replyToId })
    }]) }));
  ...
  const inferenceCanBypassBox = canSendInferenceWithoutBox(inferenceRouter);
  const showRootEmptyWorkspace = bridge != null
    && account?.kind === "logged-in"
    && (transport === "connected" || transport === "down" || boxConnectionDegraded)
    && (hasLoadedAgents || boxConnectionDegraded || transport === "down")
    && agents.length === 0
    && activeAgent == null
    && workspaceRoute == null
    && !onboardingOpen;
  ...
  return (
    <div className="sand-shell" data-empty={activeAgent == null ? true : undefined} data-loading={showRootLoading || undefined} data-runtime={bridge == null ? "browser" : "electron"} data-theme={RUNTIME_THEME_CLASS[resolvedTheme]} style={{ height: "100%", position: "relative", width: "100%" }}>
      <WorkspaceIndicator platform={bridge.platform} isFullscreen={windowFullscreen} label={workspaceRoute == null ? activeAgent?.name ?? null : null} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: `${renderedSidebarLayout.isCollapsed ? SIDEBAR_LAYOUT_BOUNDS.collapsedWidth : renderedSidebarLayout.expandedWidth}px minmax(0, 1fr)`, height: "100%", minHeight: 0, width: "100%" }}>
        <div style={{ display: "grid", gridTemplateRows: "minmax(0, 1fr) auto auto auto", minHeight: 0 }}>
          ...
          <div className="sand-agents-sidebar__account sand-agents-sidebar__footer">
            <UpdatePill bridge={bridge} labels={UPDATE_PILL_LABELS} />
            <SettingsSidebarAction
              isCollapsed={renderedSidebarLayout.isCollapsed}
        ...
        /></Suspense></main> : showRootEmptyWorkspace ? <RootShellEmptyWorkspace isVisible /> : activeAgent == null ? null : <div style={{ display: "flex", flexDirection: "column", minHeight: 0, minWidth: 0, width: "100%" }}>
          ...
          {showTranscriptLoadError
            ...
            : <ConversationTranscript
                ...
                isTransportDown={transport === "down" && !inferenceCanBypassBox}
