...
import { mergeTranscriptEntries } from "../recovered/features/conversation/workspace/transcript-order";

export { mergeTranscriptEntries } from "../recovered/features/conversation/workspace/transcript-order";
...
export function projectRendererAgent(value: unknown, now = Date.now()): RendererAgent | null {
  ...
  const name = stringValue(source.name) ?? "New chat";
  const awaitingUserResponse = source.awaitingUserResponse ?? null;
  const lastEntry = parseRendererAgentLastEntry(source.lastEntry);
  const lastMessagePreview = typeof source.lastMessagePreview === "string" ? source.lastMessagePreview : null;
  ...
  return {
    ...
    ...(typeof source.isSharedRoom === "boolean" ? { isSharedRoom: source.isSharedRoom } : {}),
    ...(typeof source.avatarDataUrl === "string" ? { avatarDataUrl: source.avatarDataUrl } : source.avatarDataUrl === null ? { avatarDataUrl: null } : {}),
    ...(typeof source.avatarVersion === "string" ? { avatarVersion: source.avatarVersion } : source.avatarVersion === null ? { avatarVersion: null } : {}),
    ...(typeof source.avatarShape === "string" ? { avatarShape: source.avatarShape } : source.avatarShape === null ? { avatarShape: null } : {}),
    ...(typeof source.avatarColor === "string" ? { avatarColor: source.avatarColor } : source.avatarColor === null ? { avatarColor: null } : {}),
    ...(source.currentActivity === undefined ? {} : { currentActivity: source.currentActivity }),
    ...(isRecord(source.remoteRoom) && typeof source.remoteRoom.roomId === "string" && source.remoteRoom.roomId.length > 0
      ? { remoteRoom: { roomId: source.remoteRoom.roomId } }
      : source.remoteRoom === null ? { remoteRoom: null } : {}),
    ...(typeof source.sharedRoomId === "string" && source.sharedRoomId.length > 0 ? { sharedRoomId: source.sharedRoomId } : {}),
    ...(source.hasUnread === true ? { hasUnread: true } : {}),
    updatedAt: numberValue(source.updatedAt, now),
    ...(source.isPinned === true ? { isPinned: true } : {}),
    ...(source.isRunning === true ? { isRunning: true } : {}),
    ...(isRecord(awaitingUserResponse) && typeof awaitingUserResponse.reason === "string"
      ? { waitingReason: awaitingUserResponse.reason }
      : {}),
    ...(typeof source.draftPrompt === "string" ? { draftPrompt: source.draftPrompt } : {}),
    ...(typeof source.isComposingMessage === "boolean" ? { isComposingMessage: source.isComposingMessage } : {}),
    lastEntry,
    ...(typeof source.lastMessageId === "string" || source.lastMessageId === null ? { lastMessageId: source.lastMessageId } : {}),
    lastMessagePreview,
    ...(lastMessage == null ? {} : { lastMessage }),
    isGroup: source.isGroup === true,
    isHidden: source.isHiddenFromSidebar === true || source.hiddenFromSidebar === true,
    memberIds: stringArray(source.memberIds ?? source.members),
    conversationPartnerIds: stringArray(source.conversationPartnerIds),
    awaitingUserResponse,
    raw: source as RendererAgentRaw
  };
}

export function projectRendererAgents(value: unknown, now = Date.now()): RendererAgent[] {
  const rows = isRecord(value) && Array.isArray(value.agents) ? value.agents : value;
...
function isRoutedTranscriptEntry(entry: ConversationTranscriptEntry): boolean {
  return (entry as ConversationTranscriptEntry & { readonly routed?: unknown }).routed === true;
}

/**
 * A host snapshot is authoritative for remote entries, but it does not know
 * about external-provider turns persisted by the local coordinator. Preserve
 * those routed (and still-pending) entries while replacing remote state.
 */
export function mergeTranscriptBaselineEntries(
  currentEntries: readonly ProductionTranscriptEntry[],
  baselineEntries: readonly ProductionTranscriptEntry[],
): ProductionTranscriptEntry[] {
  const baselineIds = new Set(baselineEntries.map((entry) => entry.id));
  const preserved = currentEntries.filter((entry) => (
    !baselineIds.has(entry.id)
    && (isRoutedTranscriptEntry(entry) || entry.id.startsWith("pending-"))
  ));
  return mergeTranscriptEntries(preserved, baselineEntries);
}
...
export function projectTranscriptFeedEntries(
  ...
  const projected = values.flatMap((value, index) => {
    const projected = projectTranscriptEntry(value, index, agentName, agentId);
    return projected == null ? [] : [projected];
  });
  return mergeTranscriptEntries([], projected);
}
...
export function projectTranscriptPageResult(value: unknown, agentName: string, agentId?: string | null): ProjectedTranscriptPage {
  ...
  const entries = mergeTranscriptEntries([], value.entries
