...
import type { DesktopBridge, DesktopUpdateStatus, InferenceRouterSnapshot, ThemePreference } from "../../../contracts/desktop-bridge";
import {
...
import { DEFAULT_ROUTER_PROVIDER, type RouterProviderId } from "./router";
...
export function SettingsDesktopSurface({ bridge, coordinatorClient = null, initialSection = "general", isOpen, onClose, onNotice, onStatus, computer }: SettingsDesktopSurfaceProps) {
  ...
  const [routerSnapshot, setRouterSnapshot] = useState<InferenceRouterSnapshot | null>(null);
  const [routerPending, setRouterPending] = useState(false);
  const [routerModelPending, setRouterModelPending] = useState(false);
  ...
  useEffect(() => {
    ...
    void bridge.agent.getInferenceRouter().then((value) => {
      if (!active) return;
      setRouterSnapshot(value);
      setRouterProvider(value.provider);
      setRouterModel(value.model);
      setRouterModels(Array.isArray(value.models) ? value.models : []);
    }).catch(() => {
      if (!active) return;
      setRouterProvider(DEFAULT_ROUTER_PROVIDER);
      setRouterSnapshot(null);
      setRouterModel(null);
      setRouterModels([]);
    });
    return () => { active = false; };
        ...
        if (section === "router") return (
          <RouterSettingsPanel
            onChange={async (provider) => {
              ...
              try {
                const next = await bridge.agent.setInferenceRouter(provider);
                setRouterSnapshot(next);
                setRouterProvider(next.provider);
                setRouterModel(next.model);
                setRouterModels(Array.isArray(next.models) ? next.models : []);
            ...
            model={routerModel}
            models={routerModels}
            modelPending={routerModelPending}
            onModelChange={async (model) => {
              if (routerModelPending) return;
              setRouterModelPending(true);
              try {
                const next = await bridge.agent.setInferenceModel(model, routerProvider);
                setRouterSnapshot(next);
                setRouterProvider(next.provider);
                setRouterModel(next.model);
                setRouterModels(Array.isArray(next.models) ? next.models : []);
                ...
                const message = reason instanceof Error ? reason.message : String(reason);
                publishSurfaceNotice({ kind: "error", operation: "settings-router-model", message }, handleNotice, onStatus);
              } finally {
                setRouterModelPending(false);
              }
            }}
            pending={routerPending}
            provider={routerProvider}
            local={routerSnapshot?.local ?? null}
          />
        );
        return (
