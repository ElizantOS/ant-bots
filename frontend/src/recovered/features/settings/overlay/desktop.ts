...
export async function loadSettingsDesktopSnapshot(bridge: DesktopBridge, coordinatorClient?: Pick<ProductionCoordinatorClient, "isEgressTunnelAvailable">): Promise<SettingsDesktopSnapshot> {
  // Keep the first settings paint independent from optional usage, update,
  // security-key, and egress services. Agent controls remain visible and are
  // read from their local stores in parallel with the account/theme state.
  const [status, avatar, autoReview, theme, timeZone, localToolPermission] = await Promise.all([
  ...
  return {
    ...
    update: null,
    usageSummary: null,
    usage: { status: "unavailable", summary: null },
    usagePageFeatureGateEnabled: false,
    egressTunnel: { available: false, featureGateEnabled: false, enabled: false, status: { state: "off" } }
  };
}
