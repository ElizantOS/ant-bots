...
export interface RouterSettingsPanelProps {
  ...
  onModelChange(model: string | null): void | Promise<unknown>;
}

export function RouterSettingsPanel({ provider, pending = false, model, models, modelPending = false, local, onChange, onModelChange }: RouterSettingsPanelProps) {
  const selectedProvider = routerProviderById(provider);
  const [modelDraft, setModelDraft] = useState(model ?? "");
  useEffect(() => setModelDraft(model ?? ""), [model, provider]);
  const localStatus = local?.[provider] as { installed?: unknown; authenticated?: unknown } | undefined;
  const statusText = localStatus?.authenticated === true
    ? "Local CLI is ready"
    : localStatus?.installed === true
      ? "CLI installed; sign in from its terminal command"
      : provider === "openrouter"
        ? "Use an OpenRouter API key in Secrets"
        : "CLI not detected on this Mac";
  return (
    <div className="sand-router-section">
      <SettingsGroup title="Provider">
        ...
        <label className="sand-settings-row">
          <span className="sand-settings-copy">
            <strong>Model</strong>
            <small>{model == null ? `Use ${selectedProvider.label}'s default model` : `Current model: ${model}`}</small>
          </span>
          <div className="sand-router-model-control">
            {models.length > 0 ? <SandSelect
              ariaLabel="Provider model presets"
              className="ui-select-trigger"
              disabled={pending || modelPending}
              menuSize="md"
            ...
            <input
              ...
              className="ui-select-trigger sand-router-model-input"
              disabled={pending || modelPending}
            ...
            />
            <SandButton
              disabled={pending || modelPending || modelDraft.trim() === (model ?? "")}
              onClick={() => void onModelChange(modelDraft.trim().length === 0 ? null : modelDraft.trim())}
              size="sm"
              variant="secondary"
            >
              {modelPending ? "Saving…" : "Save"}
            </SandButton>
          </div>
        </label>
        <div className="sand-settings-row"><span className="sand-settings-copy"><strong>Status</strong><small>{statusText}</small></span></div>
      </SettingsGroup>
