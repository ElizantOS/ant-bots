...
// @evidence src/app/package.json#xlsx=^0.18.5
...
// The release chunk is the browser build of the same npm dependency. Keep
// this boundary lazy: spreadsheet parsing is only needed when a file opens.

export async function loadShippedSpreadsheetRuntime(): Promise<SpreadsheetRuntime> {
  const module = await import("xlsx");
  const runtime = module.default ?? module;
  if (typeof runtime.read !== "function" || typeof runtime.utils?.sheet_to_json !== "function") {
    throw new Error("The spreadsheet parser is unavailable.");
  }
  return runtime as SpreadsheetRuntime;
}
