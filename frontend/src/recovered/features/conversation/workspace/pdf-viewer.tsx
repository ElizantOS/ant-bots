...
export const PDFJS_ASSET = "pdfjs-dist";
export const PDF_WORKER_FILENAME = "pdf.worker.min.mjs";
export const PDF_WORKER_ASSET = import.meta.env?.DEV === true
  ? `/__reconstructed/${PDF_WORKER_FILENAME}`
  : `./assets/pdfjs/${PDF_WORKER_FILENAME}`;
...
export function resolvePdfWorkerUrl(runtimeAssetUrl = PDF_WORKER_ASSET, baseUrl = documentBaseUrl()): string {
  const runtimeUrl = new URL(runtimeAssetUrl, baseUrl);
  if (runtimeAssetUrl === PDF_WORKER_ASSET || runtimeUrl.pathname.endsWith(PDF_WORKER_FILENAME)) return runtimeUrl.href;
...
export async function loadShippedPdfRuntime(): Promise<PdfRuntime> {
  ...
  if (!workerConfigured) {
    runtime.GlobalWorkerOptions.workerSrc = PDF_WORKER_ASSET;
