...
export interface TranscriptCardEntryBase {
  ...
  /** Internal marker for messages persisted by the external-provider router. */
  routed?: boolean;
}
...
export function projectTranscriptCardEntry(value: unknown): TranscriptCardEntry | null {
  ...
  if (value.routed !== undefined && typeof value.routed !== "boolean") return null;
  ...
  return {
    ...
    ...(value.routed === undefined ? {} : { routed: value.routed }),
  };
}
