...
export function WindowChrome({ bridge, isFullscreen, isMaximized, isOverlayTone = false }: WindowChromeProps) {
  ...
  if (bridge.platform === "darwin") return <>{dragRegion}{isFullscreen ? null : <div aria-label="Window controls" className="sand-window-controls sand-window-controls--mac" role="group">
    <button aria-label="Close" className="sand-window-control sand-window-control--close" onClick={() => void bridge.windowControls.close().catch(() => undefined)} title="Close" type="button" />
    <button aria-label="Minimize" className="sand-window-control sand-window-control--minimize" onClick={() => void bridge.windowControls.minimize().catch(() => undefined)} title="Minimize" type="button" />
    <button aria-label={isMaximized ? "Restore" : "Maximize"} className="sand-window-control sand-window-control--maximize" onClick={() => void bridge.windowControls.toggleMaximize().catch(() => undefined)} title={isMaximized ? "Restore" : "Maximize"} type="button" />
  </div>}</>;
