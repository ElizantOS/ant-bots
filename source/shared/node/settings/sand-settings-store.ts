...
import { emptySandInferenceRouterUsage, isExternalSandInferenceProvider, isSandInferenceProvider, resolveSandInferenceProviderDefault, resolveSandInferenceProviderOverride, SAND_INFERENCE_PROVIDER_DEFAULT_ENV, type SandInferenceProvider, type SandInferenceRouterUsage } from "../../inference-router.js";
import { DEFAULT_SAND_BOX_RUNTIME, isSandBoxRuntime, type SandBoxRuntime } from "../../box-runtime.js";
import { getLocalInferenceCliStatus } from "../inference-router-local.js";
...
export interface SandStoredSettings {
  ...
  inferenceProvider?: SandInferenceProvider; inferenceModels?: Partial<Record<SandInferenceProvider, string>>; inferenceRouterUsage?: SandInferenceRouterUsage;
...
function normalizeInferenceModel(value: unknown): string | undefined {
...
function parseSettings(value: unknown): SandStoredSettings | null {
  ...
  if (typeof raw.inferenceModels === "object" && raw.inferenceModels != null && !Array.isArray(raw.inferenceModels)) {
    const models: Partial<Record<SandInferenceProvider, string>> = {};
    for (const provider of Object.keys(raw.inferenceModels as Record<string, unknown>)) {
      if (!isSandInferenceProvider(provider)) continue;
      const model = normalizeInferenceModel((raw.inferenceModels as Record<string, unknown>)[provider]);
      if (model !== undefined) models[provider] = model;
    }
    if (Object.keys(models).length > 0) result.inferenceModels = models;
  }
...
export class SandSettingsStore {
  ...
  getBoxRuntime(): SandBoxRuntime {
    const override = process.env.SAND_BOX_RUNTIME?.trim().toLowerCase();
    if (isSandBoxRuntime(override)) return override;
    const stored = this.load().boxRuntime;
    if (stored !== undefined) return stored;
    const provider = this.getInferenceProvider();
    return isExternalSandInferenceProvider(provider) || provider === "cursor" && getLocalInferenceCliStatus().cursor.authenticated
      ? "local-docker"
      : DEFAULT_SAND_BOX_RUNTIME;
  }
  ...
  getInferenceProvider(): SandInferenceProvider {
    ...
    const local = getLocalInferenceCliStatus();
    const startupDefault = resolveSandInferenceProviderOverride({ SAND_INFERENCE_PROVIDER: process.env[SAND_INFERENCE_PROVIDER_DEFAULT_ENV] });
    return resolveSandInferenceProviderOverride(process.env) ?? resolveSandInferenceProviderDefault({
      stored: stored ?? startupDefault,
      isPackaged: process.env.SAND_PACKAGED === "1",
      cursorAgentAvailable: local.cursor.authenticated,
      codexAuthenticated: local.codex.authenticated,
      claudeCodeAuthenticated: local["claude-code"].authenticated,
      openCodeAuthenticated: local.opencode.authenticated,
    });
  }
  setInferenceProvider(value: SandInferenceProvider): void { this.update((s) => ({ ...s, inferenceProvider: value })); }
  getInferenceModels(): Partial<Record<SandInferenceProvider, string>> { return { ...(this.load().inferenceModels ?? {}) }; }
  getInferenceModel(provider = this.getInferenceProvider()): string | undefined { return this.load().inferenceModels?.[provider]; }
  setInferenceModel(provider: SandInferenceProvider, value?: string): void {
    this.update((settings) => {
      const models = { ...(settings.inferenceModels ?? {}) };
      const normalized = normalizeInferenceModel(value);
      if (normalized === undefined) delete models[provider]; else models[provider] = normalized;
      const { inferenceModels: _old, ...rest } = settings;
      return Object.keys(models).length === 0 ? rest : { ...rest, inferenceModels: models };
    });
  }
  setInferenceModels(value: Partial<Record<SandInferenceProvider, string>>): void {
    const models: Partial<Record<SandInferenceProvider, string>> = {};
    for (const provider of Object.keys(value)) {
      if (!isSandInferenceProvider(provider)) continue;
      const model = normalizeInferenceModel(value[provider as SandInferenceProvider]);
      if (model !== undefined) models[provider] = model;
    }
    this.update((settings) => {
      const { inferenceModels: _old, ...rest } = settings;
