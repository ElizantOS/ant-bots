...
import { createCursorAgentAuthStatus, createExternalRouterAuthStatus, isExternalSandInferenceProvider } from "../shared/inference-router.js";
import {
...
export function createDesktopPreloadBridge(options: {
  ...
  const effectiveCursorAuthStatus = async (status: any): Promise<any> => {
    try {
      const router = await edge("getInferenceRouter");
      const provider = router?.provider;
      if (provider === "cursor" && router?.local?.cursor?.authenticated === true) return createCursorAgentAuthStatus();
      if (isExternalSandInferenceProvider(provider)) return createExternalRouterAuthStatus(provider);
    } catch {
      // Router state is unavailable only during the short startup handoff; keep the real account status.
    }
    return status;
  };
  const getCursorAuthStatus = async (): Promise<any> => await effectiveCursorAuthStatus(await edge("getCursorAuthStatus"));
  const onCursorAuthChanged = (listener: (payload: unknown) => void): (() => void) => subscribe("cursor-auth-changed", (payload) => { void effectiveCursorAuthStatus(payload).then(listener); });
  const desktop: Record<string, any> = {
    ...
    cursorAccount: {
      ...
      onStatusChanged: onCursorAuthChanged,
    },
    experiments: {
    ...
    agent: {
      ...
      setInferenceModel: (model: string | null, provider?: string) => edge("setInferenceModel", { model, ...(provider == null ? {} : { provider }) }),
