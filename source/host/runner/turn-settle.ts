...
export function createTurnSettle(
  ...
  let transcriptBaseState: TurnCheckpoint | undefined;
  let transcriptRecovery: Promise<void> | undefined;
  ...
  function noteBaseState(
    ...
    transcriptBaseState = baseState;
    transcriptRecovery = undefined;
  }

  async function recoverTranscriptIfNeeded(context: unknown): Promise<void> {
    const mirror = host.transcriptMirror;
    const baseState = transcriptBaseState;
    if (!transcriptPersistenceEnabled || mirror?.recover == null || baseState == null) return;
    if (transcriptRecovery == null) {
      transcriptRecovery = mirror.recover(
        context,
    ...
    try {
      await transcriptRecovery;
    } catch (error) {
      // A transient recovery failure must be retried by the next checkpoint.
      transcriptRecovery = undefined;
      throw error;
    }
  }
  ...
  async function persistCheckpoint(
    ...
    if (preparedTranscriptMirror != null) {
      await recoverTranscriptIfNeeded(context);
