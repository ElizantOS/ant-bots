...
import { createProviderPromptSession, isRoutedProviderReady } from "../extensions/inference/provider-session.js";
...
export async function createTurnAgentRunContext<ContextValue>(
  ...
  const useCursorBackend = inferenceProvider === "cursor" && !isRoutedProviderReady(inferenceProvider);
  const agent = useCursorBackend
  ...
  const summarizationSession = useCursorBackend ? input.inference.createSummarizationSession?.(
    input.onRequestId,
    {
