...
import { createProviderPromptSession, isRoutedProviderReady } from "./provider-session.js";
...
export function createHostInference(options: HostInferenceOptions) {
  ...
  return {
    ...
    createSession(onRequestId: (requestId: string) => void, sessionOptions?: Parameters<typeof cursor.createSession>[1]) {
      ...
      if (provider === "cursor" && !isRoutedProviderReady(provider)) return routedSession(cursor.createSession(onRequestId, sessionOptions), provider);
    ...
    createSummarizationSession(onRequestId: (requestId: string) => void, sessionOptions?: Parameters<NonNullable<typeof cursor.createSummarizationSession>>[1]) {
      const provider = routerSettings.getInferenceProvider();
      if (provider === "cursor" && !isRoutedProviderReady(provider)) return routedSession(cursor.createSession(onRequestId, { ...(sessionOptions ?? {}), isSummarizationSession: true }), provider) as ReturnType<NonNullable<typeof cursor.createSummarizationSession>>;
