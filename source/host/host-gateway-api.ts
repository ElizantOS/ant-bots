...
import { createRoutedAgentToolOwner } from "./agents/routed-agent-tools.js";
...
export function createHostGatewayApi(
  ...
  const routedAgentTools = createRoutedAgentToolOwner({
    listAgents: async () => await method(manager, "listAgents")(),
    createBackgroundAgent: async (profile) => await method(manager, "createBackgroundAgent")(profile, "user"),
    updateAgent: async (agentId, profile) => await method(manager, "updateAgent")(agentId, profile),
    sendToAgent: async (fromAgentId, toAgentId, message, images, priority) => await method(manager, "sendToAgent")(fromAgentId, toAgentId, message, images, priority),
  });
  ...
  return {
    ...
    listRoutedAgentTools: (args: any) => routedAgentTools.listTools(args?.agentId),
    executeRoutedAgentTool: (args: any) => routedAgentTools.execute(args),
