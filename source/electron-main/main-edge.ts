...
import { isSandInferenceProvider, SAND_INFERENCE_MODEL_PRESETS, type SandInferenceProvider } from "../shared/inference-router.js";
import { getCachedLocalInferenceCliStatus, refreshLocalInferenceCliStatus } from "../shared/node/inference-router-local.js";
...
function parseInferenceModel(value: unknown): string | undefined {
  if (typeof value !== "string") return undefined;
  const model = value.trim();
  return model.length === 0 || model.length > 256 || /[\s\p{Cc}]/u.test(model) ? undefined : model;
}

function inferenceRouterSnapshot(deps: MainEdgeDeps, provider: SandInferenceProvider, settings?: UnknownRecord) {
  const localModels = invoke(deps.settingsStore, "getInferenceModels");
  const localUsage = invoke(deps.settingsStore, "getInferenceRouterUsage");
  return {
    provider,
    model: invoke(deps.settingsStore, "getInferenceModel", provider) ?? null,
    models: [...new Set([
      ...SAND_INFERENCE_MODEL_PRESETS[provider],
      ...((settings?.inferenceModels as Record<string, unknown> | undefined)?.[provider] && typeof (settings?.inferenceModels as Record<string, unknown>)[provider] === "string"
        ? [(settings?.inferenceModels as Record<string, unknown>)[provider] as string]
        : (localModels as Record<string, unknown> | undefined)?.[provider] && typeof (localModels as Record<string, unknown>)[provider] === "string"
          ? [(localModels as Record<string, unknown>)[provider] as string]
          : []),
    ])],
    usage: settings?.inferenceRouterUsage ?? localUsage ?? null,
    // Reading router state is on the settings click path. Never synchronously
    // spawn a provider CLI here; a background refresh keeps this snapshot warm.
    local: getCachedLocalInferenceCliStatus(provider === "cursor" || provider === "codex" || provider === "claude-code" || provider === "opencode" ? provider : undefined),
  };
}

function refreshInferenceRouterCliStatus(provider: SandInferenceProvider): void {
  const localProvider = provider === "cursor" || provider === "codex" || provider === "claude-code" || provider === "opencode" ? provider : undefined;
  void refreshLocalInferenceCliStatus(localProvider).catch(() => {
    // Credential status is advisory; a failed background probe must not affect
    // provider selection or turn execution.
  });
}
...
export function createMainEdgeHandlers(deps: MainEdgeDeps): HandlerMap {
  const handlers: HandlerMap = {
    ...
    // Router state is local to this desktop. Do not wait for the optional box
    // transport here: settings and provider switching must stay responsive even
    // when the remote computer is reconnecting.
    getInferenceRouter: () => { const provider = invoke(deps.settingsStore, "getInferenceProvider"); const selected = isSandInferenceProvider(provider) ? provider : "cursor"; refreshInferenceRouterCliStatus(selected); return inferenceRouterSnapshot(deps, selected); },
    setInferenceRouter: async (raw) => { const provider = req(raw).provider; invariant(isSandInferenceProvider(provider), "Unknown inference provider."); invoke(deps.settingsStore, "setInferenceProvider", provider); void deps.syncHostSettingsToBox({ inferenceProvider: provider }).catch((error) => reportDesktopEdgeFailure("host-settings", "inference-provider", error)); refreshInferenceRouterCliStatus(provider); return inferenceRouterSnapshot(deps, provider); },
    setInferenceModel: async (raw) => { const request = req(raw); const current = invoke(deps.settingsStore, "getInferenceProvider"); const provider = request.provider === undefined ? current : request.provider; invariant(isSandInferenceProvider(provider), "Unknown inference provider."); const model = request.model === null ? undefined : parseInferenceModel(request.model); invariant(request.model === null || model !== undefined, "Model id must be a non-empty string without whitespace."); invoke(deps.settingsStore, "setInferenceModel", provider, model); void deps.syncHostSettingsToBox({ inferenceModels: invoke(deps.settingsStore, "getInferenceModels") }).catch((error) => reportDesktopEdgeFailure("host-settings", "inference-model", error)); refreshInferenceRouterCliStatus(provider); return inferenceRouterSnapshot(deps, provider); },
