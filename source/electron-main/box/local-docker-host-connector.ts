...
const LOCAL_DOCKER_PRIMARY_NOVNC_CONTAINER_PORT = 6_080;
const LOCAL_DOCKER_FORK_NOVNC_CONTAINER_PORT = 6_081;
const LOCAL_DOCKER_PRIMARY_NOVNC_ENV = "SAND_BOX_PRIMARY_NOVNC_PORT";
const LOCAL_DOCKER_FORK_NOVNC_ENV = "SAND_BOX_FORK_NOVNC_PORT";
...
export function hasExpectedLocalDockerPortBindings(ports: unknown): boolean {
  if (typeof ports !== "object" || ports == null || Array.isArray(ports)) return false;
  const primaryNovncPort = configuredLocalDockerPort(LOCAL_DOCKER_PRIMARY_NOVNC_ENV, LOCAL_DOCKER_PRIMARY_NOVNC_CONTAINER_PORT);
  const forkNovncPort = configuredLocalDockerPort(LOCAL_DOCKER_FORK_NOVNC_ENV, LOCAL_DOCKER_FORK_NOVNC_CONTAINER_PORT);
  return [[1337, 1337], [1339, 1339], [1340, 1340], [primaryNovncPort, LOCAL_DOCKER_PRIMARY_NOVNC_CONTAINER_PORT], [forkNovncPort, LOCAL_DOCKER_FORK_NOVNC_CONTAINER_PORT], [8790, 8790]].every(([hostPort, containerPort]) => {
    const bindings = Reflect.get(ports, `${containerPort}/tcp`);
    if (!Array.isArray(bindings)) return false;
    return bindings.some((binding) => (
      typeof binding === "object"
      && binding != null
      && Reflect.get(binding, "HostIp") === "127.0.0.1"
      && Reflect.get(binding, "HostPort") === String(hostPort)
    ));
  });
}

function configuredLocalDockerPort(name: string, fallback: number): number {
...
async function inspectContainer(): Promise<{ exists: boolean; running: boolean; owned: boolean; image: string; hostSha256: string; gatewayTokenSha256: string; hasInferenceCredential: boolean; inferenceProvider: string; schemaVersion: string; publishedPortsReady: boolean }> {
  const result = await runDocker(["inspect", "--format", "{{json .}}", LOCAL_DOCKER_BOX_CONTAINER]);
  if (!result.ok) return { exists: false, running: false, owned: false, image: "", hostSha256: "", gatewayTokenSha256: "", hasInferenceCredential: false, inferenceProvider: "", schemaVersion: "", publishedPortsReady: false };
  try {
    const value = JSON.parse(result.output) as { State?: { Running?: unknown }; Config?: { Image?: unknown; Labels?: Record<string, unknown> }; NetworkSettings?: { Ports?: unknown } };
    return {
      exists: true,
      ...
      publishedPortsReady: hasExpectedLocalDockerPortBindings(value.NetworkSettings?.Ports),
    };
...
export async function getLocalDockerStatus(settingsPath: string): Promise<LocalDockerStatus> {
  ...
  const gatewayTokenMatches = inspected.gatewayTokenSha256 === hashGatewayToken(token);
  const ready = inspected.running && inspected.publishedPortsReady && gatewayTokenMatches && await gatewayReady(token);
  const detail = ready
    ? "Local Docker VM is ready."
    : !gatewayTokenMatches
      ? "Container credentials do not match this development profile; the local VM will be recreated on next connect."
    : !inspected.publishedPortsReady
      ? "Container ports are not mapped; the local VM will be recreated on next connect."
      : inspected.running
...
async function ensureLocalDockerBox(settingsPath: string, inferenceCredential?: InferenceCredential): Promise<GatewayConnection> {
  const token = await readOrCreateToken(settingsPath);
  const gatewayTokenSha256 = hashGatewayToken(token);
  const inferenceProvider = new SandSettingsStore(settingsPath).getInferenceProvider();
  const primaryNovncPort = configuredLocalDockerPort(LOCAL_DOCKER_PRIMARY_NOVNC_ENV, LOCAL_DOCKER_PRIMARY_NOVNC_CONTAINER_PORT);
  const forkNovncPort = configuredLocalDockerPort(LOCAL_DOCKER_FORK_NOVNC_ENV, LOCAL_DOCKER_FORK_NOVNC_CONTAINER_PORT);
  ...
  if (inspected.exists && (!inspected.publishedPortsReady || inspected.schemaVersion !== LOCAL_DOCKER_SCHEMA_VERSION || inspected.hostSha256 !== hostBundle.sha256 || inspected.gatewayTokenSha256 !== gatewayTokenSha256 || inspected.inferenceProvider !== inferenceProvider || (inferenceCredential != null && !inspected.hasInferenceCredential))) {
  ...
  const shouldReplace = inspected.exists && (!inspected.publishedPortsReady || inspected.schemaVersion !== LOCAL_DOCKER_SCHEMA_VERSION || inspected.hostSha256 !== hostBundle.sha256 || inspected.gatewayTokenSha256 !== gatewayTokenSha256 || inspected.inferenceProvider !== inferenceProvider || (inferenceCredential != null && !inspected.hasInferenceCredential));
  ...
  if (current.exists && !current.running) {
    ...
    const created = await runDocker([
      ...
      "--env", `${LOCAL_DOCKER_PRIMARY_NOVNC_ENV}=${primaryNovncPort}`,
      "--env", `${LOCAL_DOCKER_FORK_NOVNC_ENV}=${forkNovncPort}`,
      "--publish", "127.0.0.1:1337:1337", "--publish", "127.0.0.1:1339:1339", "--publish", "127.0.0.1:1340:1340",
      "--publish", `127.0.0.1:${primaryNovncPort}:${LOCAL_DOCKER_PRIMARY_NOVNC_CONTAINER_PORT}`, "--publish", `127.0.0.1:${forkNovncPort}:${LOCAL_DOCKER_FORK_NOVNC_CONTAINER_PORT}`, "--publish", "127.0.0.1:8790:8790",
