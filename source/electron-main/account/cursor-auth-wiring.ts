...
import { createCursorAgentAuthStatus, createExternalRouterAuthStatus, isExternalSandInferenceProvider, type SandInferenceProvider } from "../../shared/inference-router.js";
...
export function createCursorAuthWiring(deps: {
  ...
  readonly getInferenceProvider?: () => SandInferenceProvider;
  ...
  const effectiveStatus = (status: SandAuthStatus): SandAuthStatus => {
    const provider = deps.getInferenceProvider?.();
    if (provider === "cursor" && getLocalInferenceCliStatus().cursor.authenticated) return createCursorAgentAuthStatus();
    return provider != null && isExternalSandInferenceProvider(provider)
      ? createExternalRouterAuthStatus(provider)
      : status;
  };
  ...
  function deliverCursorAuthStatus(service: AuthServicePort, status: SandAuthStatus): void {
    ...
    deps.emitAuthStatus({ ...effectiveStatus(status), freshness: authStatusFreshness });
  ...
  async function ensureCursorAuthService(): Promise<AuthServicePort> {
    ...
    unsubscribeAuthStatus = service.subscribe((status) => {
      ...
      else runtime.observe(effectiveStatus(status));
    });
...
export function createCursorAccountEdgePort(deps: {
  ...
  const routedStatus = (): SandAuthStatus | null => {
    const provider = deps.getInferenceProvider?.();
    if (provider === "cursor" && getLocalInferenceCliStatus().cursor.authenticated) return createCursorAgentAuthStatus();
    return provider != null && isExternalSandInferenceProvider(provider) ? createExternalRouterAuthStatus(provider) : null;
  };
  const routedAccess = (): SandAccess | null => routedStatus() == null ? null : { state: "granted", reason: "none" };
  const settledStatus = async (getStatus: () => Promise<SandAuthStatus>) => {
    // Routed providers have their own credential/session boundary. Waiting for
    // the Cursor coordinator account transition here would keep the renderer
    // on the signed-out gate even though the selected provider is usable.
    const provider = deps.getInferenceProvider?.();
    if (provider != null && (isExternalSandInferenceProvider(provider) || provider === "cursor" && getLocalInferenceCliStatus().cursor.authenticated)) return effectiveStatus(await getStatus());
    return effectiveStatus(await deps.getAccountRuntime()?.whenIdle() ?? await getStatus());
  };
  ...
  return {
    getSandAccess: async () => routedAccess() ?? await (await ensureSandAccessReader()).read(),
    getSandAccessFresh: async () => routedAccess() ?? (await readSandAccessOnce(await sandAccessDeps())).access,
    getAuthStatus: async () => { const freshness = deps.currentAuthStatusFreshness(); const routed = routedStatus(); if (routed != null) return { ...routed, freshness }; const service = await deps.ensureCursorAuthService(); return { ...await settledStatus(() => service.getStatus()), freshness }; },
    login: async () => { const routed = routedStatus(); if (routed != null) return routed; return await withService(async (service) => { const result = await service.login(); const settled = await deps.getAccountRuntime()?.whenIdle(); await deps.resetMcpManager(); await deps.refreshHostMcp(); return settled ?? result; }); },
    cancelLogin: async () => { const routed = routedStatus(); if (routed != null) return routed; return await withService(async (service) => { const result = await service.cancelLogin(); return await deps.getAccountRuntime()?.whenIdle() ?? result; }); },
    logout: async () => { const routed = routedStatus(); if (routed != null) return routed; return await withService(async (service) => { const result = await service.logout(); return await deps.getAccountRuntime()?.whenIdle() ?? result; }); },
    ...
    getAvatar: async () => { if (routedStatus() != null) return null; return await withService(async (service) => { const status = await service.getStatus(); return status.kind !== "logged-in" || status.authId == null ? null : await deps.resolveAvatar(status.authId, status.profilePictureUrl); }); },
    getWeeklyUsage: async () => routedStatus() != null ? null : await withService(async (service) => (await service.getStatus()).kind === "logged-in" ? await deps.fetchWeeklyUsage(tokenReader(service)) : null),
    getUsageSummary: async () => routedStatus() != null || !await deps.isUsagePageEnabled() ? null : await withService(async (service) => (await service.getStatus()).kind === "logged-in" ? await deps.fetchUsageSummary(tokenReader(service)) : null),
    getPrReviewPreferences: async () => routedStatus() != null ? NO_SAND_PR_REVIEW_PREFERENCES : await withService(async (service) => (await service.getStatus()).kind === "logged-in" ? await deps.fetchPrReviewPreferences(tokenReader(service)) : NO_SAND_PR_REVIEW_PREFERENCES),
    getPrivacyModeEnabled: async () => routedStatus() != null ? true : await withService(async (service) => (await service.getStatus()).kind === "logged-in" ? await deps.fetchPrivacyModeEnabled(tokenReader(service)) : true),
    cancelTrial: async () => routedStatus() != null || !await deps.isUsagePageEnabled() ? { ok: false, message: "This isn’t available for a local provider" } : await withService(async (service) => (await service.getStatus()).kind === "logged-in" ? await deps.cancelTrial(tokenReader(service)) : { ok: false, message: "Sign in to Cursor to continue" }),
    invokeDashboardAction: async (raw: unknown) => {
      ...
      if (routedStatus() != null) return { ok: false, message: "This action belongs to the Cursor account and is unavailable for a local provider" };
