...
export interface MainBrowserWindow extends WindowStatePersistenceWindow {
  ...
  focus(): void;
  close(): void;
  /** macOS only; optional so test doubles and non-mac hosts remain valid. */
...
const STARTUP_SPLASH_BACKGROUND = "#f7f7f7";
const STARTUP_SPLASH_URL = `data:text/html;charset=utf-8,${encodeURIComponent(`<!doctype html>
<html><head><meta charset="utf-8"><style>
html,body{width:100%;height:100%;margin:0;overflow:hidden}
body{align-items:center;background:${STARTUP_SPLASH_BACKGROUND};color:#242424;display:flex;flex-direction:column;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;justify-content:center;user-select:none}
.mark{align-items:center;background:#111;border-radius:7px;display:flex;height:46px;justify-content:center;margin-bottom:20px;transform:rotate(45deg);width:46px}
.mark span{background:#fff;border-radius:4px;height:18px;transform:rotate(-45deg);width:5px}
.mark span+span{margin-left:7px}
.status{font-size:18px;line-height:24px}
</style></head><body><div class="mark" aria-hidden="true"><span></span><span></span></div><div class="status">Starting Grok Bot...</div></body></html>`)}`;
const RENDERER_COMMIT_WAIT_SCRIPT = `new Promise((resolve) => {
  const deadline = Date.now() + 5000;
  const check = () => {
    if (document.querySelector(".sand-shell") || document.querySelector("#root > *")) {
      resolve(true);
      return;
    }
...
async function waitForRendererCommit(window: MainBrowserWindow): Promise<void> {
  try {
    await window.webContents.executeJavaScript(RENDERER_COMMIT_WAIT_SCRIPT, false);
  } catch {
    // A renderer navigation failure is reported by the normal startup path.
  }
}

function hideMacWindowButtons(window: MainBrowserWindow, platform: NodeJS.Platform): void {
...
export function startElectronMain(deps: ElectronMainDependencies): ElectronMainRuntime {
  ...
  let startupSplashWindow: MainBrowserWindow | undefined;
  let startupSplashFailure: string | null = null;

  const updateStartupSplashFailure = (message: string): void => {
    startupSplashFailure = message;
    const splash = startupSplashWindow;
    if (splash == null || splash.isDestroyed()) return;
    const script = `(() => { const status = document.querySelector(".status"); if (status) status.textContent = ${JSON.stringify(message)}; })()`;
    void splash.webContents.executeJavaScript(script, false).catch(() => {});
  };

  const closeStartupSplashWindow = (): void => {
    const splash = startupSplashWindow;
    startupSplashWindow = undefined;
    if (splash == null || splash.isDestroyed()) return;
    splash.close();
  };

  const createStartupSplashWindow = (): void => {
    if (startupSplashWindow != null && !startupSplashWindow.isDestroyed()) return;
    const placement = deps.windowStatePersistence.resolveSandWindowPlacement();
    const splash = deps.createBrowserWindow({
      ...placement.windowOptions,
      title: deps.appName,
      backgroundColor: STARTUP_SPLASH_BACKGROUND,
      ...windowChromeOptions({ isMac: platform === "darwin", isWindows: platform === "win32", backgroundColor: STARTUP_SPLASH_BACKGROUND }),
      webPreferences: {
    ...
    hideMacWindowButtons(splash, platform);
    splash.on("closed", () => {
      if (startupSplashWindow === splash) startupSplashWindow = undefined;
    });
    splash.show();
    void splash.loadURL(STARTUP_SPLASH_URL).then(() => {
      if (startupSplashFailure != null) updateStartupSplashFailure(startupSplashFailure);
    }).catch(() => {});
  };
  ...
  const createWindow = async (): Promise<void> => {
    ...
    await waitForRendererCommit(window);
  };
  ...
  void deps.app
    ...
    .then(async () => {
      ...
      if (moveDisposition === "stop-bootstrap") {
    ...
    .catch((error: unknown) => {
      updateStartupSplashFailure(`Grok Bot could not start: ${error instanceof Error ? error.message : String(error)}`);
      deps.startup.noteFailed(error);
    });
