...
export async function verifyChecksumPinnedRendererPackage({
  ...
  try {
    ...
    for (const row of parsed.chunks) {
      ...
      if (relative == null || !expectedFiles.has(relative) || chunks.has(relative) || !["registry", "panel", "login", "account-menu", "window-chrome", "onboarding"].includes(row.role)
    ...
    if (chunks.size < 1 || chunks.size > 6) throw new Error("Renderer extension chunk cardinality is invalid");
