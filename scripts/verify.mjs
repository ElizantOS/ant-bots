...
try {
  const extension = JSON.parse(extractFile(builtAsar, "dist/renderer-router-extension.json").toString("utf8"));
  for (const row of extension.chunks ?? []) {
    if (typeof row?.path === "string" && row.path.startsWith("dist/renderer/") && row.patched != null) {
      rendererExtensionChunks.set(row.path.slice("dist/renderer/".length), row.patched);
    }
  }
} catch {
  // The artifact renderer remains valid without an optional extension manifest.
}
...
if (rendererComposition?.mode === "clean-source") {
  const forbiddenRendererAssets = rendererListing.filter(entry => entry === "dist/renderer/assets/index-UbX-y3il.js");
