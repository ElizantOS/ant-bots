...
async function validateEvidenceClosure() {
  // `recover-frontend` writes a useful forensic report, but it is generated
  // output and can describe an older patched artifact. Deterministic source
  // builds must consume the checked-in closure unless a caller explicitly
  // requests a live audit. This keeps repeated dev/package builds stable and
  // prevents stale ignored files from changing the accepted source graph.
  const liveAudit = process.env.SAND_RENDERER_LIVE_AUDIT === "1";
  if (!liveAudit || !existsSync(path.join(repoRoot, "recovered", "frontend", "reports", "imports.tsv"))) {
...
export async function copyPdfRuntimeAssets(rendererRoot) {
  const packageRoot = path.join(repoRoot, "node_modules", "pdfjs-dist");
  const packageManifest = JSON.parse(await readFile(path.join(packageRoot, "package.json"), "utf8"));
  if (packageManifest.version !== PDFJS_VERSION) {
    throw new Error(`pdfjs-dist package drifted: expected ${PDFJS_VERSION}, found ${packageManifest.version}`);
  }
  const source = path.join(packageRoot, "build", PDF_WORKER_FILENAME);
  const bytes = await readFile(source);
  const relative = `assets/pdfjs/${PDF_WORKER_FILENAME}`;
  const target = path.join(rendererRoot, relative);
  await mkdir(path.dirname(target), { recursive: true });
  await writeFile(target, bytes);
  return { version: PDFJS_VERSION, file: relative, bytes: bytes.byteLength, sha256: sha256(bytes) };
}
...
export async function buildProductionRenderer({ outputRoot, sourceOnly = false }) {
  ...
  const emittedDynamicEntries = [...(viteManifest["index.html"]?.dynamicImports ?? [])].sort();
  const expectedLazyEntries = bootstrap.lazyBoundaries.map(boundary => boundary.cleanDynamicEntry).sort();
  const expectedLazyEntrySet = new Set(expectedLazyEntries);
  const emittedLazyEntries = emittedDynamicEntries.filter((entry) => expectedLazyEntrySet.has(entry)).sort();
  const emittedDependencyEntries = emittedDynamicEntries.filter((entry) => !expectedLazyEntrySet.has(entry)).sort();
